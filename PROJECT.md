...
| 8 | **个人中心功能** | 用户可查看自己发布的失物信息、自己发布的招领信息、认领记录；并可对未完成信息进行修改、删除。 | **部分实现**（profile.html 可查看我的失物/招领 + 匹配 + 编辑/删除 + **我提交的认领申请（认领记录）四分区已实现**：get_my_claims 已对接 solve 表，卡片含招领信息+申请时间+发布者+对应失物+状态徽章+双详情跳转；其余认领审核结果记录等待功能7完成） |
| 9 | **消息通知功能** | 认领申请提交、审核通过、审核被拒绝三种情况后，系统向相关用户发送消息提醒。 | **部分实现**（当前已实现的消息事件：a)匹配通知 matched_notifications.type=match；b)评论通知 comments is_read=0；c)注册/找回/注销/评论/匹配 邮件；d)**认领申请提交通知（站内 + 邮件）** 已实现（matched_notifications.type=claim + 招领主人 get_messages.php claim 子查询 UNION + mark_message_read 支持 type=claim + PHPMailer try/catch 隔离邮件）；审核通过/拒绝两种通知待功能7实现） |
