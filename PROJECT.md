...
| 6 | **认领申请功能** | 失主可以对招领信息提交认领申请，并填写：物品特征、丢失经过、其他验证信息。 | **已实现**（submit_claim.php：POST FormData 5 参数 + 事务 FOR UPDATE 双表锁行 + INSERT solve status=processing + UNIQUE(lost,found,user) 防重复提交 HTTP 409；get_claims_for_my_found.php 招领主人查收到申请列表；get_my_claims.php 失主查我提交的申请历史；详情页招领主人 renderIncomingClaimsList 三要素卡+processing徽章；失主 renderClaimForm 提交表单+已申请状态卡；消息中心 matched_notifications.type=claim 认领申请通知分类；个人中心 profile.html 认领记录分区） |
| 7 | **认领审核功能** | 发布招领信息的用户可以查看认领申请、根据申请内容选择通过或拒绝。 | **已实现**（review_claim.php：POST solve_id+decision approve/reject + 事务 FOR UPDATE 锁 solve+found+lost 行 + approve 级联 6 写 solve→completed+solved_at + found.status=claimed + lost.status=solved + matched_notifications claim_approved INSERT IGNORE + 失主认领成功邮件 try/catch 隔离；reject 级联 4 写 solve→rejected + claim_rejected 通知 + 邮件；招领主人非本人=403、非 processing 态审核=409、参数缺失=400、approve 时招领/失物状态不匹配=409；详情页招领主人视角 renderIncomingClaimsList processing 卡底部「通过/拒绝」双按钮 + window.confirm 二次确认 + handleReviewClaim 调用 API 后自动刷新列表；completed/rejected 态申请只读不渲染按钮） |
| 8 | **个人中心功能** | 用户可查看自己发布的失物信息、自己发布的招领信息、认领记录；并可对未完成信息进行修改、删除。 | **已实现**（a) 顶部用户信息概览卡：首字母渐变圆形头像 + 用户名/姓名/学号/电话/邮箱/注册时间 + 邮箱认证状态绿橙双色徽章 + 一键跳转独立 user_info.html 管理资料修改密码；b) 6项实时数据统计卡：失物 pending/solved 各 N 条 + 招领 unclaimed/claimed 各 N 条 + 匹配物品 N 条 + 认领记录 N 条（数字来自实际三个 API 返回结果计算，并非前端虚构）；c) profile.html 四大分区：我的失物 pending/solved 双分区 / 我的招领 unclaimed/claimed 双分区 / 匹配度较高的物品 / 全部认领记录（三态：处理中/认领成功/审核未通过），分区标题已从二态改为三态，避免误导；d) pending/unclaimed 未完成态显示 编辑/删除 按钮：编辑跳转 edit_listing.html、删除 confirm 二次确认 + deleteListing API；e) 整卡点击跳转 addCardClickListeners 对失物/招领/匹配三个分区统一生效，认领记录四分区补充整卡点击跳招领详情（排除 action-btn），交互风格一致；f) 认领记录卡片"查看对应失物"按钮从 btn-edit 黄色编辑语义修正为 btn-view 蓝色查看语义，三态徽章 processing/completed/rejected 三色 + 审核时间 solved_at 展示 + 拒绝态红色文案提示；g) 修复了原 profile.js 查询 profile.html 中并不存在的 matchNotification/matchDot DOM 导致 ReferenceError 的潜在崩溃 Bug；h) 顶部导航可跳转 user_info.html 独立页面修改资料/改密码/改安全问题/注销） |
| 9 | **消息通知功能** | 认领申请提交、审核通过、审核被拒绝三种情况后，系统向相关用户发送消息提醒。 | **已实现**（完整覆盖六种消息事件+UI完善+安全加固：a)匹配通知 type=match；b)评论通知 comment；c)认领申请通知 type=claim；d)认领审核通过通知 type=claim_approved；e)认领审核拒绝通知 type=claim_rejected；f)注册/找回/注销/评论/匹配/认领提交/认领通过/认领拒绝 全邮件链路；g)**消息中心顶部分类型统计摘要**：5枚彩色stat-chip（总未读蓝/匹配黄/评论青/认领申请紫/审核结果绿），由 get_messages.php 新增 debug.count_by_type + count_review_total 字段驱动；h)**认领申请/审核通过/审核拒绝三类型CSS视觉识别**：claim=紫色边条 claim_approved=绿色边条+浅绿底 claim_rejected=红色边条+浅粉底（原2类无样式导致视觉无法区分）；i)**桌面端消息中心2×2 grid布局**：@media≥800px时四分区2列，手机保持单列flex；j)**全局导航新增消息中心入口+未读红点徽章**：auth.js installMessageUnreadBadge 自动在 user-nav登出按钮前插入「消息中心」链接+右上角红色未读数字徽章（1-99真实数字、≥100显示99+），每45秒轮询刷新；k)**markMessageRead 封装**：messages.js 从直接fetch改为调用 api/index.js markMessageRead；l)**安全加固：评论消息所有权校验**：mark_message_read.php comment分支 UPDATE 原缺 user_id 权限判断 → 改为 JOIN lost/found 表 UPDATE ... WHERE l.user_id = 当前登录id，禁止任何登录用户越权标记他人物品评论已读；m) messages.html 四分区点击mark_read + 跳 details 详情 + 未登录重定向 login.html 全部正常；n)主页 index.html 顶部 banner 原「您有新的消息待查看」提醒文字保留与导航红点并存） |
| 10 | **管理员功能** | 管理员可审核用户发布的信息、删除虚假或违规内容、管理物品分类、管理用户账号。 | **部分实现（已完成通用CRUD安全加固+独立会话闭环+主站入口+role校验）**（a)管理员独立会话闭环：新增 logout.php 销毁 admin_logged_in/admin_user_id SESSION键 + session_regenerate_id；dashboard.js 登出按钮改为真实调用 logout 接口；checkAdminAuth() 从空壳 return true→改为 fetch get_tables.php 真实 HTTP 探测 401 立即跳登录页；b)**update_row 敏感字段黑名单**（G1高危修复）：users 表 password_hash/security_answer/security_question/verification_code 四列禁止修改，列名白名单循环内 throw Exception + 大小写不敏感判断；c)**delete_row 安全加固**（G2高危修复）：pk_name 先用 INFORMATION_SCHEMA TABLE_CONSTRAINTS + KEY_COLUMN_USAGE 校验为该表真实主键（prepared），伪造主键名直接 401 禁止；table=users 且 pk_value==SESSION.admin_user_id 时拦截「禁止删除当前登录的管理员账号」，避免锁库；d)字段支撑：get_user_info.php SELECT 补 role；check_session.php 连表查 users.role 写入返回 user 对象（默认'user'兜底），未登录不查；e)主站管理后台入口：auth.js installAdminDashboardLink(userNav,user) 当 user.role==='admin' 时，在 logoutBtn beforebegin 插入「管理后台」a 标签跳 admin/dashboard.html，与消息中心视觉一致；f)**debug_notifications 管理员权限固化**（AGENTS.md §17已知隐患）：新增 SELECT role WHERE user_id=? prepared 校验，非 admin 返回 403「权限不足：此接口仅管理员可访问」；g)现有通用表级CRUD保留：11表白名单 SHOW TABLES 防注入；列名 SHOW COLUMNS 白名单；主键用 INFORMATION_SCHEMA 探测；模态框编辑 + confirm 删除；h)**明确未做三项**（避免虚构功能）：1.独立信息审核流程页（DB无status审核字段不做ALTER）2.独立分类管理页（无categories表）3.用户禁用/重置密码独立页（无ban字段无独立按钮）。分类/禁用/重置可通过通用表级CRUD在 users/lost/found 三表白名单内手动操作） |
