...
| 9 | **消息通知功能** | 认领申请提交、审核通过、审核被拒绝三种情况后，系统向相关用户发送消息提醒。 | **已实现**（完整覆盖三种消息事件：a)匹配通知 matched_notifications.type=match；b)评论通知 comments is_read=0；c)注册/找回/注销/评论/匹配 邮件；d)**认领申请提交通知** type=claim 站内+邮件；e)**认领审核通过通知** type=claim_approved 站内（get_messages.php claim_approved 子查询 UNION + mark_message_read 支持四类型）+ 邮件；f)**认领审核拒绝通知** type=claim_rejected 站内（claim_rejected 子查询同构）+ 邮件；消息中心 messages.html 新增独立「认领审核结果消息」分区区分认领申请与审核结果，点击 mark_read + 跳对应招领详情） |
