...
CREATE TABLE `matched_notifications` (
  ...
  `type` enum('match','claim','claim_approved','claim_rejected') NOT NULL DEFAULT 'match' COMMENT '消息类型：match=系统匹配通知，claim=认领申请通知，claim_approved=认领审核通过，claim_rejected=认领审核拒绝',
  `is_read` tinyint(1) DEFAULT '0',
  `created_at` datetime DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `user_id` (`user_id`),
...
CREATE TABLE `solve` (
  ...
  `status` enum('processing','completed','rejected') NOT NULL DEFAULT 'processing' COMMENT '申请状态: processing=待审核, completed=审核通过, rejected=审核拒绝',
